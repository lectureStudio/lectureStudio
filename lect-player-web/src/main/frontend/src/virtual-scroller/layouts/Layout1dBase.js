export class Layout1dBase {
  ...
  addEventListener(type, listener, options) {
    ...
    this._listeners[type].push({ target: this, listener: listener, options: options })
  }
  ...
  dispatchEvent(event) {
    ...
    for (let i = 0, l = stack.length; i < l; i++) {
      if (typeof stack[i].listener === 'function')
        stack[i].listener.call(stack[i].target, this);
      else
        stack[i].listener.handleEvent(event);
    }
