...
import org.lecturestudio.presenter.api.presenter.StreamViewPresenter;
 ...
 * @author Alex Andres
 */
public class ViewStreamHandler extends CefStreamHandler {

	private final EventBus eventBus;


	public ViewStreamHandler(PresenterContext context) {
		super(context);
	...
	public void initialize() {
		context.viewStreamProperty().addListener((o, oldValue, newValue) -> {
			if (newValue) {
				try {
					if (!isJcefInstalled()) {
						installJcef();
	...
	private void showStreamView() {
		eventBus.post(new ShowPresenterCommand<>(StreamViewPresenter.class));
	}

	private void hideStreamView() {
		eventBus.post(new ClosePresenterCommand(StreamViewPresenter.class));
	}
}
