...
public class WebSocketConnector extends Connector {
	...
	private static final Logger LOG = LogManager.getLogger(WebSocketConnector.class);
	...
	public WebSocketConnector(StreamDescription streamDescription) {
	...
	public void send(Object packet) {
		if (writeHandler != null) {
			if (packet instanceof ByteBuf) {
				...
				writeHandler.send(new BinaryWebSocketFrame(channelBuffer));
			}
			else {
	...
	protected void initInternal() throws ExecutableException {
		...
		try {
			...
			bootstrap.group(bossGroup, workerGroup)
					...
					.childHandler(new WebSocketInitializer(handlers, tlsEnabled));
		}
		catch (Exception e) {
			String message = "Failed to init " + getClass().getName();
			panic(message, e);
		}
	}
