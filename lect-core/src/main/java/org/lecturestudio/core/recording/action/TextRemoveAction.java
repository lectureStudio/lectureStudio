...
public class TextRemoveAction extends PlaybackAction {

	private int handle;


	public TextRemoveAction(int handle) {
		this.handle = handle;
	}

	public TextRemoveAction(byte[] input) throws IOException {
		parseFrom(input);
	}

	@Override
	public void execute(ToolController controller) throws Exception {
		controller.removeText(handle);
	}
