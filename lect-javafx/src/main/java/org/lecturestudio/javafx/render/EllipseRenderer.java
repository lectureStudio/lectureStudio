...
public class EllipseRenderer implements Renderer<GraphicsContext> {
	...
	public void render(Shape shape, GraphicsContext context) {
		...
		if (keyEvent != null && keyEvent.isAltDown()) {
			...
			context.fillOval(bounds.getX(), bounds.getY(), bounds.getWidth(), bounds.getHeight());
		}
		else {
			context.setLineWidth(width);
			context.setStroke(color);
			context.strokeOval(bounds.getX(), bounds.getY(), bounds.getWidth(), bounds.getHeight());
		}
		...
		if (shape.isSelected()) {
			if (keyEvent != null && keyEvent.isAltDown()) {
				...
				context.fillOval(bounds.getX() + width, bounds.getY() + width, bounds.getWidth() - 2 * width, bounds.getHeight() - 2 * width);
			}
			else {
