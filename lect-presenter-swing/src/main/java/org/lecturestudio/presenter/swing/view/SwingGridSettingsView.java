...
@SwingView(name = "grid-settings", presenter = org.lecturestudio.presenter.api.presenter.GridSettingsPresenter.class)
public class SwingGridSettingsView extends JPanel implements GridSettingsView {
	...
	@Inject
	SwingGridSettingsView(RenderController renderer) {
		super();
	...
	public void setGridColor(ObjectProperty<Color> color) {
		SwingUtils.bindBidirectional(gridColorPicker, new ConvertibleObjectProperty<>(color,
	...
	public void setGridInterval(DoubleProperty interval) {
		...
		SwingUtils.bindBidirectional(gridLinesSlider, new ConvertibleNumberProperty<>(interval, converter));
	}
